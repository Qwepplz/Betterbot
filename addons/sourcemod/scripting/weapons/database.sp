...
void ResetPlayerWeaponData(int clientIndex, int weaponIndex, int team) {
  g_iSkins[clientIndex][weaponIndex][team] = 0;
  g_iStatTrak[clientIndex][weaponIndex][team] = 0;
  g_iStatTrakCount[clientIndex][weaponIndex][team] = 0;
  g_NameTag[clientIndex][weaponIndex][team] = "";
  g_fFloatValue[clientIndex][weaponIndex][team] = 0.0;
  g_iWeaponSeed[clientIndex][weaponIndex][team] = -1;
}

void LoadPlayerWeaponData(DBResultSet results, int clientIndex, int weaponIndex, int team) {
  ResetPlayerWeaponData(clientIndex, weaponIndex, team);

  char weaponName[32];
  RemoveWeaponPrefix(g_WeaponClasses[weaponIndex], weaponName, sizeof(weaponName));

  char baseFieldName[64];
  if (team == CS_TEAM_CT) {
    FormatEx(baseFieldName, sizeof(baseFieldName), "ct_%s", weaponName);
  } else {
    strcopy(baseFieldName, sizeof(baseFieldName), weaponName);
  }

  char fieldName[64];
  int fieldIndex;

  if (results.FieldNameToNum(baseFieldName, fieldIndex)) {
    g_iSkins[clientIndex][weaponIndex][team] = results.FetchInt(fieldIndex);
  }

  FormatEx(fieldName, sizeof(fieldName), "%s_float", baseFieldName);
  if (results.FieldNameToNum(fieldName, fieldIndex)) {
    g_fFloatValue[clientIndex][weaponIndex][team] = results.FetchFloat(fieldIndex);
  }

  FormatEx(fieldName, sizeof(fieldName), "%s_trak", baseFieldName);
  if (results.FieldNameToNum(fieldName, fieldIndex)) {
    g_iStatTrak[clientIndex][weaponIndex][team] = results.FetchInt(fieldIndex);
  }

  FormatEx(fieldName, sizeof(fieldName), "%s_trak_count", baseFieldName);
  if (results.FieldNameToNum(fieldName, fieldIndex)) {
    g_iStatTrakCount[clientIndex][weaponIndex][team] = results.FetchInt(fieldIndex);
  }

  FormatEx(fieldName, sizeof(fieldName), "%s_tag", baseFieldName);
  if (results.FieldNameToNum(fieldName, fieldIndex)) {
    results.FetchString(fieldIndex, g_NameTag[clientIndex][weaponIndex][team], 128);
  }

  FormatEx(fieldName, sizeof(fieldName), "%s_seed", baseFieldName);
  if (results.FieldNameToNum(fieldName, fieldIndex)) {
    g_iWeaponSeed[clientIndex][weaponIndex][team] = results.FetchInt(fieldIndex);
  }
}
    ...
    if (results == null) {
      ...
      if (results.FetchRow()) {
        for (int weaponIndex = 0; weaponIndex < sizeof(g_WeaponClasses); weaponIndex++) {
          ...
          LoadPlayerWeaponData(results, clientIndex, weaponIndex, CS_TEAM_CT);
        }
